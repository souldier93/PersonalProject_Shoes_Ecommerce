      ...
      <div class="flex items-center justify-between">
        ...
        <nav class="hidden items-center gap-1 overflow-x-auto lg:flex">
          <router-link v-for="item in menuItems" :key="item.path" :to="item.path" class="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors hover:bg-gray-100" :class="isActive(item.path) ? 'bg-gray-900 text-white hover:bg-gray-800' : 'text-gray-700'">
            ...
            <svg v-else-if="item.name === 'Inventory'" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7h16M4 12h16M4 17h16M6 7v10m12-10v10" />
            </svg>
            ...
            <svg v-else-if="item.name === 'Chat'" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const menuItems = [
  ...
  { name: "Inventory", label: "Inventory", path: "/admin/inventory" },
  { name: "Purchases", label: "Purchases", path: "/admin/purchases" },
  { name: "Chat", label: "Chat", path: "/admin/chat" },
